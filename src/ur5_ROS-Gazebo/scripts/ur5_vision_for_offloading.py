...
class ur5_vision:
    def __init__(self):
        rospy.init_node("ur5_vision_for_offloading", anonymous=False)
        ns = rospy.get_namespace().strip('/')
        print("the namespace is " + ns)
        self.track_flag = False
        self.default_pose_flag = True
        self.cx = 400.0
        self.cy = 400.0
        self.bridge = cv_bridge.CvBridge()
        self.image_sub = rospy.Subscriber('/'+ ns + '/ur5/usbcam/image_raw', Image, self.image_callback)
        self.cxy_pub = rospy.Publisher('/' + ns + '/cxy', Tracker, queue_size=1)
    ...
    def image_callback(self,msg):
        ...
        #  biggest_contour = max(contour_sizes, key=lambda x: x[0])[1]
            for i, c in enumerate(cnts):
                ...
                if area > 5000:
                    print("Area is greater than threshold")
