...
class ur5_mp:
    def __init__(self):
        ...
        self.cxy_sub = rospy.Subscriber('/' + ns + '/cxy', Tracker, self.tracking_callback, queue_size=1)
        self.cxy_pub = rospy.Publisher('/' + ns + '/cxy1', Tracker, queue_size=1)
        self.phase = 1
        self.object_cnt = 0
    ...
    def execute(self):
        ...
        if self.track_flag:
            ...
            if len(self.pointx)>8:
                if len(self.pointx)==9:
                    ...
                    y_speed = np.mean(np.asarray(self.pointy[4:8]) - np.asarray(self.pointy[3:7]))
                    ...
                    if len(self.pointx)<12:
                        ...
                        wpose.position.y += (y_speed+self.error_y*0.04/105)

                    else:
                        if tracker.flag2:
                            self.track_flag=False
                        transition_pose = deepcopy(start_pose)
                        transition_pose.position.z = -0.1000
                        print(start_pose)
                        ...
                        transition_pose.position.z = 0.1
                        print(transition_pose.position.z)
                        self.waypoints.append(deepcopy(transition_pose))
            ...
            # Set the next waypoint to the right 0.5 meters
                ...
                wpose.position.y += self.error_y*0.1/105
        ...
        # print self.points
        time_end = time.time()
        tim_diff = time_end - time_begin
        self.count = self.count + tim_diff
        print("Time taken for motion plan " + str(tim_diff))
        print("Accumulator: " + str(self.count))
